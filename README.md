...
`play`. Vosk segment-level hypotheses are stitched across partial/final boundaries
before final ranking so long commands like `play Eric Prydz Opus` stay coherent
through multiple decoder passes. A dedicated thread continuously drains 100 ms
chunks from a two-second
